...
class Cryptor:
    ...
    def generate_key(self):
        try:
            ...
            # Decoding key for saving purpose
            decoded_key = key.decode()
            file = filedialog.asksaveasfile(mode='w', filetypes=[('Key Files', '*.key')],
                                            defaultextension=[('Key Files', '*.key')])
            file.write(decoded_key)
            file.close()
        except:
            tk.messagebox.showerror(title="Error", message="An error has occurred while generating a key.")
    ...
    def create_widgets(self):
        ...
        self.tabControl.add(self.tab2, text= 'Key Generation Panel')
        self.tab3 = ttk.Frame(self.tabControl)  # Creates third tab
        self.tabControl.add(self.tab3, text='Hotkeys List')
        self.tab4 = ttk.Frame(self.tabControl)  # Creates third tab
        ...
        canvas2 = ttk.Labelframe(self.tab2, text=' File Cryptor - Key Generation Tab ')
        canvas2.grid(column=0, row=0, padx=0, pady=0)
        canvas2.pack(fill='both', expand='yes')

        # Configuration for tab3 frame
        canvas3 = ttk.Labelframe(self.tab3, text=' File Cryptor - Hotkeys List ')
        canvas3.grid(column=0, row=0, padx=0, pady=0)
        canvas3.pack(fill='both', expand='yes')

        # Configuration for tab4 frame
        canvas4 = ttk.Labelframe(self.tab4, text=' File Cryptor - About ')
        canvas4.grid(column=0, row=0, padx=0, pady=0)
        ...
        # Create Key Generation Button
        self.generate_key_btn = tk.Button(canvas2, text='Generate Key', command=lambda: self.create_thread(self.generate_key), height=3, width=25)
        self.generate_key_btn.place(x=170, y=90)
        ...
        keyboard.add_hotkey('ctrl + g', self.generate_key)
